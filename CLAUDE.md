...
│   ├── build_web.py                # compiles readings + items → docs/content/ (the phone app)
│   ├── scheduler.py                # SM-2 (upgraded from Leitner; migrates old box-state)
│   ├── quiz_runner.py              # the study loop (build first)
│   ├── supa.py                     # Supabase client (stdlib urllib) — shared state with the phone
│   ├── sync_supabase.py            # status / push / pull / flush
│   ├── make_icons.py               # generates the PWA icons (pure-stdlib PNG writer)
│   ├── test_sm2_parity.py          # docs/sm2.js must schedule identically to scheduler.py
│   └── test_web_logic.py           # parse / session / analytics / assets / views + the loop
├── docs/                           # the phone app — a static PWA, served by GitHub Pages
│   ├── index.html · style.css · app.js   # shell, styles, views + routing
│   ├── sm2.js                      # SM-2 ported from scheduler.py (parity-tested)
│   ├── session.js · analytics.js   # pure logic: session assembly, stats/calibration
│   ├── store.js · supabase.js      # offline-first state + sync; auth + REST
│   ├── config.js                   # Supabase URL + anon key (committed; public by design)
│   └── content/                    # GENERATED by build_web.py — never hand-edit
├── supabase/schema.sql             # review_state + review_log, RLS, record_attempt RPC
└── review_log.csv                  # app-written attempt log (created on first study session)
```

> `docs/` is named for GitHub Pages, which deploys a repo's `/docs` folder as-is. It is the phone
> app, not documentation — its own [`README.md`](docs/README.md) has the setup and deploy steps.
...
**Scheduling state is now shared, not local.** Since the phone app landed, the canonical copy of
scheduler state and attempt history lives in **Supabase** (`supabase/schema.sql`), so the phone and
the laptop work one queue instead of two diverging ones. `review_state.json` and `review_log.csv`
keep being written as a git-tracked snapshot and as the offline fallback — they're just no longer
the only copy. This does not change the storage decision above: content is still markdown in git,
and the *state/content* split is exactly as before. With no `.env` configured everything falls back
to local-only and behaves as it always did.
...
**The phone app (`docs/`)** obeys the same rule from the other side: it reads the *generated* bundle
in `docs/content/` and writes attempts to Supabase. It never touches `wiki/`, `items/`, or anything
else authored. Regenerate its content with `python apps/build_web.py` after any reading or item
change — treat that as part of step 6 of `create chapter`, alongside rebuilding `items.json`.

**Two SM-2 implementations now exist** — `apps/scheduler.py` and `docs/sm2.js` — and they write the
same rows. They must stay in lockstep: **if you change either, run `python apps/test_sm2_parity.py`.**
It replays ~45k transitions through both and diffs them. A silent disagreement would make an item's
schedule depend on which device happened to grade it, and nothing in normal use would reveal it.
